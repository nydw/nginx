...
static int                  ep = -1;
static struct epoll_event  *event_list; // lgx_mark 用于epoll_wait调用时传递内核态的事件
...
{
    ngx_epoll_conf_t  *epcf;  // lgx_mark 存储配置项的结构体
...
static ngx_int_t
ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags) // lgx_mark 如何调用epoll_ctl向epoll中添加事件的。
{
    int                  op;
...
static ngx_int_t
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags) //lgx_mark 实现了收集、分发事件的process_events接口的方法
{
    int                events;
    uint32_t           revents;
    ...
    for (i = 0; i < events; i++)  // // lgx_mark 遍历获取的所有事件
    {
        ...
        if ((revents & EPOLLIN) && rev->active) // 读事件且活跃
        {
            ...
            if (flags & NGX_POST_EVENTS) // 需要延后处理
            {
                queue = (ngx_event_t **) (rev->accept ?  // 判断是新连接事件还是普通事件
            ...
            {
                rev->handler(rev); // 调用事件回调方法来处理这个事件
            }
        }
...
static void
ngx_epoll_eventfd_handler(ngx_event_t *ev) // 处理已经完成异步IO事件
{
    int               n, events;
    long              i;
    uint64_t          ready;  // 表示还未处理的事件
    ngx_err_t         err;
    ngx_event_t      *e;
    ngx_event_aio_t  *aio;
    ...
    while (ready)
    {
        events = io_getevents(ngx_aio_ctx, 1, 64, event, &ts);  // 获取已经完成异步IO事件
