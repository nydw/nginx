...
struct ngx_event_s {
    void            *data;  // ngx_connection_t / ngx_event_aio_t
...
struct ngx_event_aio_s {
    void                      *data;
    ngx_event_handler_pt       handler;  // 在异步IO事件完成后被调用
    ngx_file_t                *file;
...
typedef struct {   // lgx_mark  每个事件模块需要实现的10个抽象方法
    ngx_int_t  (*add)(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags); // 添加到事件驱动机制
    ngx_int_t  (*del)(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags); // 删除事件从事件驱动机制

    ngx_int_t  (*enable)(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags);  // 启用一个事件
    ngx_int_t  (*disable)(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags); // 禁用一个事件

    ngx_int_t  (*add_conn)(ngx_connection_t *c);  //  添加连接，会同时监听读写事件
    ngx_int_t  (*del_conn)(ngx_connection_t *c, ngx_uint_t flags);    // 删除连接，会同时监听读写事件

    ngx_int_t  (*process_changes)(ngx_cycle_t *cycle, ngx_uint_t nowait); // 多线程环境下被调用。
    ngx_int_t  (*process_events)(ngx_cycle_t *cycle, ngx_msec_t timer,   // lgx_mark 调用这个方法来处理时间事件。在 ngx_process_events_and_timers 中调用。
                   ngx_uint_t flags);
...
typedef struct {
    ...
    void                 *(*create_conf)(ngx_cycle_t *cycle);  // 创建和存储配置项参数的结构体
    char                 *(*init_conf)(ngx_cycle_t *cycle, void *conf);  //  处理当前事件模块感兴趣的全部配置项
