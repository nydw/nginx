...
main(int argc, char *const *argv)
{
    ...
    if (ngx_strerror_init() != NGX_OK) { // lgx_mark 预先缓存系统错误列表
        return 1;
    }

    if (ngx_get_options(argc, argv) != NGX_OK) { // lgx_mark 解析命令参数
        return 1;
    }
...
static ngx_int_t
ngx_process_options(ngx_cycle_t *cycle)  // lgx_mark 用运行时参数初始化cycle
{
    u_char  *p;
    size_t   len;
