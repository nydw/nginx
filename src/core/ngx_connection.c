...
ngx_set_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++)
    {
        ...
        if (getsockopt(ls[i].fd, IPPROTO_TCP, TCP_DEFER_ACCEPT, &timeout, &olen)  // 检查 timeout
                == -1)
        {
            err = ngx_socket_errno;
...
ngx_int_t
ngx_open_listening_sockets(ngx_cycle_t *cycle)  //lgx_mark  打开监听端口
{
    int               reuseaddr;
...
void
ngx_configure_listening_sockets(ngx_cycle_t *cycle)  // lgx_mark 根据nginx.conf 配置文件配置监听端口
{
    int                        value;
    ngx_uint_t                 i;
    ngx_listening_t           *ls;
...
void
ngx_close_listening_sockets(ngx_cycle_t *cycle) //lgx_mark 关闭监听的端口
{
    ngx_uint_t         i;
    ngx_listening_t   *ls;
    ngx_connection_t  *c;
...
void
ngx_free_connection(ngx_connection_t *c)  // lgx_mark 回收到连接池
{
    /* ngx_mutex_lock */
