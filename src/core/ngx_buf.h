...
struct ngx_buf_s {
    u_char          *pos;
    u_char          *last;       // 表示有效的内容到此为止
    off_t            file_pos;
    off_t            file_last;  // 表示文件有效的内容到此为止

    u_char          *start;      // 这段缓冲区的开始
    u_char          *end;          
    ngx_buf_tag_t    tag;        // 表示当前缓冲区的类型，指向这个模块的ngx_module_t 
    ngx_file_t      *file;
    ngx_buf_t       *shadow;     // 指向引用相同空间的buf
    ...
    unsigned         in_file:1;   // 内容在文件
    unsigned         flush:1;
    unsigned         sync:1;
    unsigned         last_buf:1;  // 表示最后一个buf
