...
void
ngx_http_init_connection(ngx_connection_t *c) // lgx_mark listening_t->handler
{
    ngx_uint_t              i;
    ngx_event_t            *rev;
...
ngx_http_request_t *
ngx_http_create_request(ngx_connection_t *c) // lgx_mark 第一次出现可读事件时，初始化http请求
{
    ngx_pool_t                 *pool;
    ngx_time_t                 *tp;
...
static void
ngx_http_process_request_line(ngx_event_t *rev)  // lgx_mark 处理请求行
{
    ssize_t              n;
    ngx_int_t            rc, rv;
    ngx_str_t            host;
    ...
    for ( ;; )
    {
        ...
        if (rc == NGX_AGAIN)
        {
            n = ngx_http_read_request_header(r);  // lgx_mark 读取请求数据
...
static void
ngx_http_process_request_headers(ngx_event_t *rev)  // lgx_mark 接收http头部
{
    u_char                     *p;
    size_t                      len;
    ssize_t                     n;
...
void
ngx_http_process_request(ngx_http_request_t *r)  // lgx_mark 首次从业务上处理请求
{
    ngx_connection_t  *c;
...
static void
ngx_http_request_handler(ngx_event_t *ev) // lgx_mark tcp连接上后续的事件触发，处理请求
{
    ngx_connection_t    *c;
    ngx_http_request_t  *r;
    ngx_http_log_ctx_t  *ctx;
...
void
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)  // lgx_mark  结束请求
{
    ngx_connection_t          *c;
...
static void
ngx_http_terminate_request(ngx_http_request_t *r, ngx_int_t rc) // 非正常结束请求方法
{
...
static void
ngx_http_writer(ngx_http_request_t *r)  // lgx_mark 发送剩下的响应内容。
{
    int                        rc;
    ngx_event_t               *wev;
    ngx_connection_t          *c;
...
void
ngx_http_close_connection(ngx_connection_t *c) //  lgx_mark 释放tcp连接
{
    ngx_pool_t  *pool;
