...
static ngx_command_t  ngx_http_core_commands[] = {
    ...
    {   ngx_string("client_header_buffer_size"), // lgx_mark 请求头大小
        NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_CONF_TAKE1,
        ngx_conf_set_size_slot,
        NGX_HTTP_SRV_CONF_OFFSET,
...
ngx_module_t  ngx_http_core_module = { // lgx_mark http框架
    NGX_MODULE_V1,
...
void
ngx_http_handler(ngx_http_request_t *r)  // lgx_mark 真正开始处理数据
{
...
ngx_int_t
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)  // lgx_mark NGX_HTTP_POST_READ_PHASE, NGX_HTTP_LOG_PHASE,NGX_HTTP_LOG_PHASE的check方法,
{
    ngx_int_t  rc;
...
ngx_int_t
ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)  // lgx_mark NGX_HTTP_SERVER_REWRITE_PHASE, NGX_HTTP_REWRITE_PHASE的check方法
{
    ngx_int_t  rc;
...
ngx_int_t
ngx_http_core_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph) // lgx_mark  NGX_HTTP_ACCESS_PHASE的check方法
{
    ngx_int_t                  rc;
    ngx_http_core_loc_conf_t  *clcf;
...
ngx_int_t
ngx_http_core_content_phase(ngx_http_request_t *r,   // lgx_mark NGX_HTTP_CONTENT_PHASE 的checker方法
                            ngx_http_phase_handler_t *ph)
{
    size_t     root;
    ngx_int_t  rc;
...
ngx_int_t
ngx_http_send_header(ngx_http_request_t *r) // lgx_mark  构造和发送http响应行，头部
{
    if (r->header_sent) {
...
ngx_int_t
ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in) // lgx_mark 发送http包体
{
    ngx_int_t          rc;
    ngx_connection_t  *c;
