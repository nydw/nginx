...
// lgx_mark 调用filter来过滤数据，对数据进行加工，如truncked传输、gzip压缩等。
...
ngx_module_t  ngx_http_module = {   // lgx_mark  http框架
    NGX_MODULE_V1,
...
static char *
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)  //  lgx_mark   解析配置文件http
{
    char                        *rv;
    ngx_uint_t                   mi, m, s;
    ngx_conf_t                   pcf;
