...
ngx_int_t
ngx_http_upstream_create(ngx_http_request_t *r)   // lgx_mark  创建upstream
{
    ngx_http_upstream_t  *u;
...
void
ngx_http_upstream_init(ngx_http_request_t *r)   // lgx_mark 启动upstream机制
{
    ngx_connection_t     *c;
...
static void
ngx_http_upstream_check_broken_connection(ngx_http_request_t *r,  //  lgx_mark 检查nginx与下游额连接是否正常
    ngx_event_t *ev)
{
    int                  n;
    char                 buf[1];
...
static void
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u) // lgx_mark  向上游服务器发起连接
{
    ngx_int_t          rc;
    ngx_time_t        *tp;
    ngx_connection_t  *c;
...
static void
ngx_http_upstream_send_request_handler(ngx_http_request_t *r, // lgx_mark 向上游服务器发送请求
    ngx_http_upstream_t *u)
{
    ngx_connection_t  *c;
...
static void
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u) // lgx_mark 处理上游服务器的响应的头部
{
    ssize_t            n;
    ngx_int_t          rc;
    ngx_connection_t  *c;
...
static void
ngx_http_upstream_process_body_in_memory(ngx_http_request_t *r, // lgx_mark  负责接收上游服务器的包体
    ngx_http_upstream_t *u)
{
    size_t             size;
    ssize_t            n;
...
static void
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u) // lgx_mark 发送响应的包头
{
    int                        tcp_nodelay;
    ssize_t                    n;
    ngx_int_t                  rc;
...
static void
ngx_http_upstream_finalize_request(ngx_http_request_t *r, // lgx_mark  结束upstream请求
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ngx_uint_t   flush;
    ngx_time_t  *tp;
